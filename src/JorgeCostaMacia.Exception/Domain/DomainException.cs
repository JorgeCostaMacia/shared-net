...
public abstract class DomainException : System.Exception
{
    ...
    /// leveraging the <see cref="DomainExceptionDefaults"/> class for constants and <see cref="Guid.CreateVersion7()"/> for dynamic values.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    /// <param name="aggregateId">Optional unique identifier of this exception instance. <b>Generates a new GUID</b> using <see cref="Guid.CreateVersion7()"/> if null.</param>
    ...
    protected DomainException(
        ...
        string? message,
        System.Exception? innerException
    ) : base($"{aggregateId ??= Guid.CreateVersion7()}/{(aggregateType ?? DomainExceptionDefaults.AGGREGATE_TYPE).Split('.').Last()}{(string.IsNullOrEmpty(message) ? string.Empty : " => " + message?.Trim())}", innerException)
    {
