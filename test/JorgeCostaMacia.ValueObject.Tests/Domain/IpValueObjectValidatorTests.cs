...
{
    private static readonly IpValueObjectValidator _validator = IpValueObjectValidator.Create();
    ...
    public void ValidIpv4_Passes(string value)
        => Assert.True(_validator.Validate(IpValueObject.From(value)).IsValid);
    ...
    public void InvalidIp_Fails(string value)
        => Assert.False(_validator.Validate(IpValueObject.From(value)).IsValid);

    [Fact]
    public void Invalid_ValidateAndThrow_ThrowsTypedException()
        => Assert.Throws<IpValueObjectValidationException>(() => _validator.ValidateAndThrow(IpValueObject.From("999.999.999.999")));
}
