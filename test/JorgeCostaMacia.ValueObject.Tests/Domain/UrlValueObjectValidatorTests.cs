...
{
    private static readonly UrlValueObjectValidator _validator = UrlValueObjectValidator.Create();

    [Theory]
    [InlineData("https://example.com")]
    [InlineData("http://a.b/c?d=e#f")]
    public void ValidHttpUrl_Passes(string value)
        => Assert.True(_validator.Validate(UrlValueObject.From(value)).IsValid);
    ...
    public void InvalidUrl_Fails(string value)
        => Assert.False(_validator.Validate(UrlValueObject.From(value)).IsValid);
    ...
    public void Invalid_ValidateAndThrow_ThrowsTypedException()
        => Assert.Throws<UrlValueObjectValidationException>(() => _validator.ValidateAndThrow(UrlValueObject.From("notaurl")));
}
